...
object OcrSettings {
    ...
    fun getWeightRoi(ctx: Context): Roi {
        ...
        return Roi(
            p.getFloat("weight_roi_x", 0.0f), p.getFloat("weight_roi_y", 0.0f),
            p.getFloat("weight_roi_w", 1.0f), p.getFloat("weight_roi_h", 1.0f)
        )
    }
    ...
    fun saveLatestValues(ctx: Context, weight: Float?, weightAlert: Boolean) {
        ctx.getSharedPreferences(PREFS, Context.MODE_PRIVATE).edit()
